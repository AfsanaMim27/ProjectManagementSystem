...
export async function GET(request, response) {
    ...
    const [tasks, tasksFields] = await dbConnection.execute("SELECT t.TaskId, t.Title, t.ShortDescription, p.Title AS ProjectTitle, ph.Title AS PhaseTitle, t.DueDate, t.EstimatedDuration, s.Title AS TaskStatus, s.Color FROM Tasks AS t INNER JOIN Statuses AS s ON s.StatusId = t.TaskStatus INNER JOIN Projects AS p ON p.ProjectId = t.ProjectId LEFT JOIN Phases AS ph ON ph.PhaseId = t.PhaseId  where t.AssignedTo = " + contactId);
    dbConnection.end();
