...
export async function GET(request, response) {
  ...
  const [taskCounts, projectCountsFields] = await dbConnection.execute("SELECT COUNT(*) AS TotalTasks FROM Tasks WHERE Trashed = 0 " + (projectId ? ("AND ProjectId = " + projectId) : ""));
  dbConnection.end();
